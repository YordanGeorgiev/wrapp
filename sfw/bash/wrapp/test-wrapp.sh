#!/bin/bash 
# file: sfw/bash/wrapp/test-wrapp.sh
...
doRunTests(){
	...
	while read -r action ; do (
		...
		done < <(find sfw/bash/wrapp/tests -type f -name '*.sh')

		doLog "INFO STOP :: testing action: \"$action\""
	);
	done < <(cat $product_instance_dir/sfw/bash/wrapp/tests/run-wrapp-tests.lst)
...
#----------------------------------------------------------
#
# eof file: sfw/bash/wrapp/test-wrapp.sh
