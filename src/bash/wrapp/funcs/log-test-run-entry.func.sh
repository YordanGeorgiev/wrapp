# src/bash/wrapp/funcs/log-test-run-entry.func.sh
...
doLogTestRunEntry(){
   ...
   case "$1" in
     INIT)
         # cat doc/txt/wrapp/funcs/log-test-run-entry.func.txt
         ...
         && test_run_report_file="$product_instance_dir/data/tests/$component_name"'.test-run-report.'`date "+%Y%m%d_%H%M%S"`'.txt'
         echo -e "\n\n" >> "$test_run_report_file"
         echo -e `date "+%Y-%m-%d %H:%M:%S"`"  START $component_name test run report" >> "$test_run_report_file"
         echo -e "\n\n" >> "$test_run_report_file"
         echo -e "result  start-time  stop-time   action-name" >> "$test_run_report_file"
         shift;
         ;;
     START)
     ...
     INFO)
         shift;
         test_run_report_line="$test_run_report_line""$1"
         ;;
     STOP)
         shift;
         exit_code=$1
     ...
     STATUS)
         ...
         find "$product_instance_dir"'/data/tests' -type f -exec stat -c "%y %n" {} \; | sort -nr
         echo -e "\n\n"
         ;;
     *)
         echo $"Usage: $0 {START|STOP|INFO|STATUS}"
         exit 1
   esac	
...
# eof file: src/bash/wrapp/funcs/log-test-run-entry.func.sh
