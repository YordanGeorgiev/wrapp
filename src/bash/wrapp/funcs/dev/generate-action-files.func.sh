...
doGenerateActionFiles(){
			...
			while read -d" " deliverable_type ; do (
				...
				case $deliverable_type in 
					...
					#
						...
						deliverable_doc_file="doc/txt/$wrap_name/specs/$act.spec.txt"
						deliverable_code_file="src/bash/$wrap_name/specs/$act.spec.sh"
					;;
					'func')
						full_func="do""$func_part_name"
						deliverable_doc_file="doc/txt/$wrap_name/funcs/$act.func.txt"
						deliverable_code_file="src/bash/$wrap_name/funcs/$act.func.sh"
					;;
					'test')
						full_func="doTest""$func_part_name"
						deliverable_doc_file="doc/txt/$wrap_name/tests/$act.test.txt"
						deliverable_code_file="src/bash/$wrap_name/tests/$act.test.sh"
					;;
					'help')
						full_func="doHelp""$func_part_name"
						deliverable_doc_file="doc/txt/$wrap_name/helps/$act.help.txt"
						deliverable_code_file="src/bash/$wrap_name/helps/$act.help.sh"
					;;
				esac
				...
				code_file_exists=$(find "src/bash/$wrap_name/$deliverable_type""s" | grep $act.$deliverable_type.sh| wc -l)
				if [ $code_file_exists -eq 0 ];then

					cp -v src/bash/$wrap_name/funcs/%act%.%deliverable_type%.sh "$deliverable_code_file"
				...
				doc_file_exists=$(find "doc/txt/$wrap_name/$deliverable_type""s" | grep $act.$deliverable_type.txt| wc -l)
	...
	done< <(cat src/bash/$wrap_name/tests/all-wrapp-tests.lst)
